...
export class AlmocoComponent implements OnInit {
  ...
  public saveRef(){
    this.calcGramsOnRef()
    this.servicesApiService.getCalRef('al',this.emailUser).subscribe({
      next: (res)=> {
      this.servicesApiService.patchRef('al',this.emailUser,this.ref[0].id,this.massArray[0],this.ref[1].id,this.massArray[1],this.ref[2].id,this.massArray[2],this.ref[3].id,200,this.ref[4].id,1).subscribe({
  ...
  ngOnInit(): void {
    ...
    this.servicesApiService.getCalRef('al',this.emailUser).subscribe({
      next: (res)=> {
        ...
        this.servicesApiService.getRef('al',this.emailUser).subscribe({
          next: (res) =>{
          this.ref = res
          this.getNameofItens()
          },
