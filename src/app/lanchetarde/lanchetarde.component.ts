...
@Component({
  ...
  templateUrl: './lanchetarde.component.html',
  styleUrls: ['./lanchetarde.component.css']
})
export class LanchetardeComponent implements OnInit {
  ...
  public saveRef(){
    ...
    this.servicesApiService.getCalRef('lt',this.emailUser).subscribe({
      next: (res)=> {
        this.servicesApiService.patchRef('lt',this.emailUser,this.ref[0].id,this.massArray[0],this.ref[1].id,this.massArray[1],0,0,this.ref[2].id,200,this.ref[3].id,1).subscribe({
          next: (res)=> {
            console.log(res)
        },
          error: (error) => error
  ...
  ngOnInit(): void {
    ...
    this.servicesApiService.getCalRef('lt',this.emailUser).subscribe({
      next: (res)=> {
        ...
        this.servicesApiService.getRef('lt',this.emailUser).subscribe({
          next: (res) =>{
          this.ref = res
          this.getNameofItens()
          },
